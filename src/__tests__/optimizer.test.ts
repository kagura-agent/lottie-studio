...
describe("removeHiddenLayers", () => {
  it("removes hidden layers (hd=true)", () => {
    ...
    expect((result.layers as LottieTestObj[])[0].nm).toBe("b");
  });
...
describe("removeEmptyGroups", () => {
  it("removes groups with only transform items", () => {
    ...
    expect((result.layers as LottieTestObj[])[0].shapes).toHaveLength(0);
  });
  ...
  it("keeps groups with real items", () => {
    ...
    const result = removeEmptyGroups(data);
    expect((result.layers as LottieTestObj[])[0].shapes).toHaveLength(1);
  });
  ...
  it("handles layers without shapes", () => {
    ...
    const result = removeEmptyGroups(data);
    expect((result.layers as LottieTestObj[])[0]).toEqual({ nm: "text" });
  });
...
describe("removeRedundantKeyframes", () => {
  ...
  it("skips non-animated props (a!==1)", () => {
    ...
    expect((result.layers as LottieTestObj[])[0].ks.p.k).toEqual([0, 0]);
  });

  it("keeps single keyframe unchanged", () => {
    const data = { layers: [{ ks: { p: { a: 1, k: [{ t: 0, s: [0], e: [100] }] } } }] };
    const result = removeRedundantKeyframes(data);
    expect((result.layers as LottieTestObj[])[0].ks.p.k).toHaveLength(1);
  });
  ...
  it("keeps first keyframe when s !== e", () => {
    ...
    const result = removeRedundantKeyframes(data);
    expect((result.layers as LottieTestObj[])[0].ks.p.k).toHaveLength(2);
  });
  ...
  it("removes first keyframe no-op when next starts at same value", () => {
    ...
    const result = removeRedundantKeyframes(data);
    const kfs = (result.layers as LottieTestObj[])[0].ks.p.k;
  ...
  it("keeps middle keyframes with e undefined", () => {
    ...
    const result = removeRedundantKeyframes(data);
    expect((result.layers as LottieTestObj[])[0].ks.p.k).toHaveLength(3);
  });
...
describe("validateAndFix", () => {
  it("auto-fixes missing ind/ip/op/ks", () => {
    ...
    const layer = (fixed.layers as LottieTestObj[])[0];
  ...
  it("normalizes shape fill colors 0-255 → 0-1", () => {
    ...
    const { fixed, fixesApplied } = validateAndFix(data);
    const c = (fixed.layers as LottieTestObj[])[0].shapes[0].c.k;
    expect(c[0]).toBe(1);
  ...
  it("normalizes text layer fill color (fc)", () => {
    ...
    const { fixed, fixesApplied } = validateAndFix(data);
    const fc = (fixed.layers as LottieTestObj[])[0].t.d.k[0].s.fc;
    expect(fc[0]).toBe(1);
    expect(fc[1]).toBe(0);
  ...
  it("normalizes text layer stroke color (sc)", () => {
    ...
    const { fixed, fixesApplied } = validateAndFix(data);
    const sc = (fixed.layers as LottieTestObj[])[0].t.d.k[0].s.sc;
