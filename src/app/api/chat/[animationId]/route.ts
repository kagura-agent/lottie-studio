...
export async function GET(
  ...
  const messages = db.prepare(
    "SELECT id, role, content, lottie_json, created_at FROM messages WHERE animation_id = ? ORDER BY created_at ASC"
  ).all(animationId) as { id: string; role: string; content: string; lottie_json: string | null; created_at: string }[];
