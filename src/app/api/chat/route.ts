...
export async function POST(request: Request) {
  ...
  // --- Sequence commands are handled via /api/sequences routes ---
  // The /sequence subcommands (create, add, list, show, reorder, delete) are
  // managed through the REST API, not the chat SSE stream.
...
Be honest but constructive. Reference specific layers by name when possible. Do NOT generate any Lottie JSON — this is analysis only.`;
  ...
  // --- Polish command handling (before LLM call) ---
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          // Extract Lottie JSON from the response
          ...
          // Save the polished animation if we got valid Lottie JSON
          if (lottieJson) {
